...
import { loadState, saveState, putBlob, getBlob, deleteBlob } from "./utils/idb";
...
export default function App(){
  ...
  React.useEffect(()=>{
    ...
    (async ()=>{
      await saveState({ version: STORAGE_VERSION, columns, rows, npcLib, setLib, mapLib });
  ...
  const visibleRows = React.useMemo(()=>{
    ...
    data = data.filter(r => {
      ...
      const ok2 = !filters.Category || (r["Category"]||"") === filters.Category;
  ...
  function exportCSV(){
    ...
    a.href = url; a.download = "gm_campaign_export.csv"; a.click();
    URL.revokeObjectURL(url);
  }

  function exportJSONLight(){
    const json = JSON.stringify({ version: STORAGE_VERSION, columns, rows, npcLib, setLib, mapLib }, null, 2);
    const blob = new Blob([json], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a");
    a.href = url; a.download = "gm_campaign_backup_light.json"; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  async function exportJSONWithImages(){
    async function embedImages(arr){
      ...
      for (const x of arr){
        ...
        if (!img && x.imageKey){
          ...
          if (blob) img = await new Promise((res)=>{ const fr = new FileReader(); fr.onload=()=>res(fr.result); fr.readAsDataURL(blob); });
        }
        out.push({ ...x, img });
      }
      return out;
    ...
    const blob = new Blob([json], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a");
    a.href = url; a.download = "gm_campaign_backup_with_images.json"; a.click();
    URL.revokeObjectURL(url);
  }
  ...
  function onImportJSON(file){
    ...
    reader.onload = async () => {
      try{
        ...
        if (!parsed || !parsed.columns || !parsed.rows) throw new Error("Invalid backup file");
  ...
  function addRow(){
    const blank = {}; for (const c of columns) blank[c] = "";
    const rec = ensureMeta({ id: crypto.randomUUID(), ...blank });
    setRows([rec, ...rows]); setEditingId(rec.id);
  }
  ...
  async function addNpc({ name, notes, file }){
    ...
    let imageKey = null;
    if (file){ imageKey = `npc_${id}`; await putBlob(imageKey, file); }
  ...
  async function addSet({ name, notes, file }){
    const id = crypto.randomUUID();
    let imageKey = null;
    if (file){ imageKey = `set_${id}`; await putBlob(imageKey, file); }
  ...
  async function addMap({ title, notes, file }){
    const id = crypto.randomUUID();
    let imageKey = null;
    if (file){ imageKey = `map_${id}`; await putBlob(imageKey, file); }
  ...
  return (
    <div>
      ...
      <main className="container" style={{display:"grid", gap:12}}>
        {view==="tracker" && (
          <>
            ...
            <CollapsiblesPanel columns={columns} visibleRows={visibleRows} />
            <DataTable
              columns={columns}
      ...
      <footer className="container" style={{padding:"24px 16px"}}>
        <span className="muted">Autosaves to IndexedDB. Use Export (light or with images) for portable backups.</span>
      </footer>
    </div>
  );
}
